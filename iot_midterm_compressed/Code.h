        ...
        <div class="AddSensorInside">
            ...
            <div style="display: flex;justify-content: center; width: 100%;min-height: 20%;align-items: center;">
                <button onclick="editSensorPopupSave()"
                    style="margin: 10%; padding: 5px 13px;background-color: rgb(163, 255, 163);border-radius: 5px;box-shadow: rgba(100, 100, 111, 0.3) 0px 7px 29px 0px;cursor: pointer;">Save</button>
                &nbsp; &nbsp; &nbsp;
                <button onclick="popup('.EditSensor')"
...
<div class="slideThree">
    <input type="checkbox" value="None" id="slideThree` + number + `" onchange="checkbox(`+ number.toString() + `)" name="check" />
...
`
                ...
                if (chatArray[1] == "off") {
                    chatOutput.value = "There are " + (number-countOn()) + " devices off";
    ...
    var statusOfPins = [0,0,0,0,0,0,0];
    function checkbox(parm)
        {
          console.log(parm);
        statusOfPins[parseInt(parm)-1] = statusOfPins[parseInt(parm)-1]==0?1:0;
        var xhttp = new XMLHttpRequest();
        xhttp.open("GET", "led_set?field1="+statusOfPins[0]+"&field2="+statusOfPins[1]+"&field3="+statusOfPins[2]+"&field4="+statusOfPins[3], true);
        xhttp.send();
        let link = "https://api.thingspeak.com/update?api_key=Z23ESCW35RJN0OFD&field1="+statusOfPins[0]+"&field2="+statusOfPins[1]+"&field3="+statusOfPins[2]+"&field4="+statusOfPins[3];
        send(link);
    }
    function send(link) {
        var xhttp = new XMLHttpRequest();
